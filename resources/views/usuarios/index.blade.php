...
@section('contenido')
    <div class="container mx-auto px-4 py-8">
        ...
        @empty($usuarios)
            <p class="text-center text-gray-600 text-lg py-10">No existen usuarios registrados actualmente</p>
        @else
                    ...
                    <tbody>
                        @foreach($usuarios as $usuario)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                    <p class="text-gray-900 whitespace-no-wrap">{{ $usuario->roles->first()->name ?? 'Sin Rol' }}
                                    </p>
                                </td>
                                <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                    <p class="text-gray-900 whitespace-no-wrap">{{ $usuario->created_at->format('d/m/Y') }}</p>
                                </td>
                                <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm flex gap-2 items-center">
                                    ...
                                    @can('eliminar-usuario')
                                        <form action="{{ route('usuarios.destroy', $usuario->id) }}" method="POST"
                                            ...
                                            @method('DELETE')
                                            <button type="submit"
                                                class="px-3 py-1 text-sm font-semibold rounded-full bg-red-100 text-red-800 hover:bg-red-200 transition-colors duration-200">
                                                Eliminar
                                            </button>
                                        </form>
                                    @endcan
